...
const applySelfHeal = (attacker, baseAmount, scene, ability) => {
  ...
  const amount = Math.floor(modified * getMasteryMultiplier(attacker));
...
export function getWeaponSkillsFor(char) {
  ...
  for (const [id, skill] of Object.entries(SKILLS)) {
    ...
    // Gates read PROFICIENCY (permanent stats only, halved) -- not totalStats.
    // Gear no longer grants or removes access to a skill; see getProficiency
    // in CombatLogic.js. Breakthrough still bypasses it, as before.
    const statKey = skill.requiredStat?.toUpperCase();
    const statVal = statKey ? getProficiency(char, statKey) : null;
    ...
    if (skill.supersededBy && !DevFlags.isBreakthroughEnabled()) {
      ...
      if (upgrade) {
        ...
        const upVal = upKey ? getProficiency(char, upKey) : null;
...
export function getReactionSkillsFor(char) {
  ...
  for (const [id, s] of Object.entries(SKILLS)) {
    ...
    if (s.requiredStat && !DevFlags.isBreakthroughEnabled()) {
      // Proficiency, same as getWeaponSkillsFor above.
      const statVal = getProficiency(char, s.requiredStat);
...
Object.assign(RAW_SKILLS, {
  ...
  */
  ...
  'ignite': {
    ...
    requiredWeapon: ["staff"],
    requiredStat: "INT",
    requiredValue: 7,  // was 5 (even spread, 2026-09-10)
    actionCost: "reaction",
    mpCost: 3,
    cooldown: 3,
    requiresTarget: false,
...
Object.assign(RAW_SKILLS, {
  ...
  'heartpiercer': {
    ...
    requiredWeapon: ["dagger"],
    requiredStat: "DEX",
    requiredValue: 10,  // was 7 (even spread, 2026-09-10)
  ...
  'death_blow': {
    ...
    requiredWeapon: ["axe_2h"],
    requiredStat: "STR",
    requiredValue: 12,  // was 11 (even spread, 2026-09-10)
    actionCost: "major",
    mpCost: 6,
    requiresTarget: true,
    targetRequirement: "enemy",
  ...
  'bell_ringer': {
    ...
    requiredWeapon: ["mace_2h"],
    requiredStat: "STR",
    requiredValue: 10,  // was 9 (even spread, 2026-09-10)
    actionCost: "major",
    mpCost: 5,
    requiresTarget: true,
    targetRequirement: "enemy",
